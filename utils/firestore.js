...
const linkParticipantWithFirebaseUID = async (docID, firstSignInTimestamp, uid) => {
    try {
        ...
        await db.collection('participants').doc(docID).update(linkRecord);
        return true;
    }
    catch(error){
...
const linkInvitationRecordWithParticipantUID = async (docID, pin, firstSignInTimestamp, uid) => {
    try {
        ...
        await db.collection('participants').doc(docID).update(firstSignInRecord);
    }
    catch (error) {
