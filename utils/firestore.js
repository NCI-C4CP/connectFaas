...
const verifyIdentity = async (type, token, siteCode) => {
  ...
  try {
    ...
    if (snapshot.size > 0) {
      ...
      data = checkSurveyStatusAfterVerification(data);
...
const updateParticipantCorrection = async (participantData) => {
    try {
        ...
        if (participantData['state.148197146'] === 'NULL') {
            ...
            updateObj = {'state.148197146': admin.firestore.FieldValue.delete()};
        }

        if (Object.keys(participantData).length > 0 || Object.keys(updateObj).length > 0) {
            const updatedData = checkSurveyStatusAfterVerification({...docData, ...participantData, ...updateObj});
