...
/**
 * This function checks if a token exists in the participants collection and validates 
 * its status. A token is considered valid if it exists and the associated account 
 * has not yet been linked to a Firebase UID (state.uid is undefined).
 * 
 * If the participant record is marked as a duplicate account, the function sets 
 * isDuplicateAccount to true and returns immediately.
 * 
 * @async
 * @function verifyToken
 * @param {string} token - The token to verify
 * @returns {Promise<Object>} An object containing verification results:
 *   - {boolean} isDuplicateAccount - True if the account is marked as a duplicate
 *   - {boolean} isValid - True if the token is valid (participant exists and has not been linked to a Firebase uid)
 *   - {string|null} docId - Document ID of the participant record if found, null otherwise
 */
const verifyToken = async ({ token }) => {
    const resultObj = { isDuplicateAccount: false, isValid: false, docId: null };
    if (!token) return resultObj;

    const query = db.collection('participants').where('token', '==', token);
    const snapshot = await query.get();

    printDocsCount(snapshot, "verifyToken");
...
/**
 * Verify PIN number entered at sign-up. PIN numbers are sent by invitation, where a shell account with matching PIN and health care provider
 * has been created before inviting the participant to the study.
 * Search for a single document with the given pin, check whether the account has been marked as a duplicate.
 * If the account is not a duplicate, check whether the 'state' variables have been set. If not, the PIN is valid.
 * @param {string} pin - PIN number entered by participant at sign up
 * @returns {object} - Object containing duplicate account status, PIN validation status, and document ID.
 */
...
const verifyPin = async (pin) => {
    try {
        ...
        printDocsCount(snapshot, "verifyPin");
        ...
        throw new Error('Error: verifyPin', { cause: error });
    }
...
const linkParticipantWithFirebaseUID = async (docID, firstSignInTimestamp, uid) => {
    try {
        const doc = await db.collection('participants').doc(docID).get();
        const docData = doc.data();
        const stateData = docData.state || {};
        const updatedRecord = {
            ...defaultFlags,
            ...docData,
            [fieldMapping.autogeneratedSignedInTime]: firstSignInTimestamp,
        ...
        await doc.ref.update(updatedRecord);
        return true;
    }
    catch(error){
        console.error('Error in linkParticipantWithFirebaseUID:', error);
        throw new Error('Error: linkParticipantWithFirebaseUID', { cause: error });
    }
}

/**
 * When a participant signs up with a valid PIN, link the shell account with their new Firebase UID (see getToken() API)
 * Add the PIN, PIN match variable, uid, and first sign-in data to the participant's record.
 * @param {string} docID - Document ID of the participant's record
 * @param {string} pin - PIN number entered by participant at sign up
 * @param {string} firstSignInTimestamp - Timestamp of the participant's first sign-in
 * @param {string} uid - Firebase UID of the participant
 * @returns {boolean} - True if the update is successful, false if an error occurs
 */

const linkInvitationRecordWithParticipantUID = async (docID, pin, firstSignInTimestamp, uid) => {
    try {
        const doc = await db.collection('participants').doc(docID).get();
        const docData = doc.data();
        const stateData = docData.state || {};
        const updatedRecord = {
            ...defaultFlags,
            ...docData,
            [fieldMapping.pinMatch]: fieldMapping.yes,
            [fieldMapping.pinNumber]: pin,
            [fieldMapping.autogeneratedSignedInTime]: firstSignInTimestamp,
            [fieldMapping.participantMap.signedInFlag]: fieldMapping.yes,
            state: {
                ...stateData,
        ...
        console.error('Error in linkInvitationRecordWithParticipantUID:', error);
        throw new Error('Error: linkInvitationRecordWithParticipantUID', { cause: error });
    }
}
...
const updateParticipantDataByDocId = async (id, data) => {
...
const resetParticipantHelper = async (uid, saveToDb) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        let keysToPreserve = [
            ...
            fieldMapping.consentFirstName,
            fieldMapping.consentMiddleName,
            fieldMapping.consentLastName,
            fieldMapping.consentSuffixName,
            // The four user-profile name CIDs are aliased at top level as
            // firstName / middleName / lastName / suffix (same 9-digit IDs).
            ...
            fieldMapping.informedConsentDateSigned,
            fieldMapping.informedConsentVersion,
            fieldMapping.hipaaAuthorizationDateSigned,
            fieldMapping.hipaaAuthorizationFlag,
            fieldMapping.hipaaAuthorizationVersion,
            fieldMapping.authenticationEmail,
            ...
            fieldMapping.userProfileSubmittedFlag,
        ...
        obj = {
            ...
            token,
            Connect_ID,
            [fieldMapping.autogeneratedRecruitmentType]: fieldMapping.passiveRecruit, // defaulting it as passive
...
/**
 * Return the pathology bucket suffix for the active GCP project's tier.
 *
 * Bucket names are written as `pathology-reports_<site>_connect-<tier>` (see
 * utils/fileUploads.js), so `connect-<tier>` is the suffix that identifies a
 * bucket as belonging to the active deployment tier.
 */
const getPathologyBucketTierSuffix = (projectId = process.env.GCLOUD_PROJECT) => {
  if (!projectId) return "";
  // Project IDs are "nih-nci-dceg-connect-<tier>[-<hash>]". The tier segment is
  // at index 4. Mirrors the derivation used on the bucket-write side.
  const tier = projectId.split("-")[4];
  return tier ? `connect-${tier.toLowerCase()}` : "";
};

/**
 * Verify that a pathology bucket belongs to the current deployment tier before any
 * metadata is deleted.
 */
const isValidPathologyBucketName = (bucketName, projectId = process.env.GCLOUD_PROJECT) => {
  if (!bucketName || typeof bucketName !== "string") return false;
  if (!bucketName.startsWith("pathology-reports")) return false;

  const suffix = getPathologyBucketTierSuffix(projectId);
  return suffix !== "" && bucketName.toLowerCase().endsWith(suffix);
};

/**
 * Deletes pathology report files from Storage Bucket and metadata records from Firestore.
 * @param {number|string} connectId
 * @returns {Promise<string[]>} Non-empty when cleanup was skipped or incomplete.
 */
const deletePathologyReports = async (connectId) => {
  ...
  try {
    ...
    const docsEligibleForMetadataDelete = new Set();
    const docsByBucket = new Map();
    for (const doc of snapshot.docs) {
      const { bucketName } = doc.data();

      if (!isValidPathologyBucketName(bucketName)) {
        errors.push(`pathologyReports: Connect_ID ${connectId} has a metadata doc with missing or cross-tier bucketName ${JSON.stringify(bucketName)}`);
        continue;
      }
    ...
    const failedBucketSet = new Set();

    for (const [bucketName, docs] of docsByBucket.entries()) {
      const bucket = storage.bucket(bucketName);
      const [exists] = await bucket.exists();
      if (!exists) {
        errors.push(`pathologyReports: bucket ${bucketName} does not exist`);
        failedBucketSet.add(bucketName);
        continue;
      }
      ...
      for (const file of files) {
        ...
        fileDeletePromises.push(
          file.delete().catch((err) => {
            ...
            failedBucketSet.add(bucketName);
            console.error(`Failed to delete pathology report storage file for participant ${connectId} from bucket ${bucketName}:`, err);
          })
        );
      }

      docs.forEach((doc) => docsEligibleForMetadataDelete.add(doc.id));
    }
    ...
    for (const doc of snapshot.docs) {
      ...
      if (!docsEligibleForMetadataDelete.has(doc.id)) continue;
      if (failedBucketSet.has(bucketName)) continue;
...
/**
 * Process participants who requested data destruction.
 *
 * Collection and pathology cleanup must complete before the participant stub is marked destroyed.
 */
                ...
                try {
                    ...
                    if (errors.length === 0) {
                        try {
                            ...
                            await participantDocRef.update(updatedData);

                            const updatedDoc = await participantDocRef.get();
                            const updatedParticipant = updatedDoc?.exists && typeof updatedDoc.data === "function"
                                ? updatedDoc.data()
                                : null;
                            ...
                            if (updatedParticipant) {
                                ...
                                if (validation.status !== "pass") {
                                    const validationMessage =
                                        `Post-destruction validation ${validation.status} for participant ${participant["Connect_ID"]}: ` +
                                        JSON.stringify({
                                ...
                                console.error(`Unable to re-read participant ${participant["Connect_ID"]} after data destruction update for validation`);
                            }
...
const verifyIdentity = async (type, token, siteCode) => {
  ...
  try {
    ...
    if (snapshot.size > 0) {
      ...
      const docData = doc.data();
      const existingValue = docData[verificationStatus];
      ...
      if (newValue === fieldMapping.verified) {
          ...
          if (docData[fieldMapping.birthMonth.toString()] && docData[fieldMapping.birthDay.toString()] && docData[fieldMapping.birthYear.toString()]) {
              let dobString = docData[fieldMapping.birthMonth.toString()] + '/' + docData[fieldMapping.birthDay.toString()]  + '/' + docData[fieldMapping.birthYear.toString()];
      ...
      const dataToBeUpdated = checkSurveyStatusesWhenVerified(data, docData);
      await doc.ref.update(dataToBeUpdated);
      return true;
    } else {
...
const queryCountReplacementHomeCollectionAddressesToPrint = async () => {
    try {
        ...
        const query1 = `${collectionDetails}.${baseline}.${bioKitMouthwashBL1}.${kitStatus}`;
        const query2 = `${collectionDetails}.${baseline}.${bioKitMouthwashBL2}.${kitStatus}`;

        const [snapshot1, snapshot2] = await Promise.all([
            db.collection('participants').where(query1, '==', initialized).count().get(),
            db.collection('participants').where(query2, '==', initialized).count().get(),
        ]);
...
const queryReplacementHomeCollectionAddressesToPrint = async (limit) => {
    try {
        ...
        const query1 = db.collection('participants')
                    .where(path1, '==', initialized)
                    .get();

        const query2 = db.collection('participants')
                    .where(path2, '==', initialized)
                    .get();
...
const queryKitsByReceivedDate = async (receivedDateTimestamp) => {
    ...
    const maxSize = 30;
    do {
        const inArr = kitIds.slice(start, Math.min(maxSize, kitIds.length));
        if(!inArr.length) {
            break;
        }
        let query = db.collection('kitAssembly')
            .where(`${fieldMapping.uniqueKitID}`, 'in', inArr);
...
const eligibleParticipantsForKitAssignment = async () => {
    try {
        ...
        // There should never be redundant participants but just in case
...
const addKitStatusToParticipantV2 = async (participants) => {
    try {
        const { collectionDetails, baseline, bioKitMouthwash, bioKitMouthwashBL1, bioKitMouthwashBL2, kitStatus, addressPrinted } = fieldMapping;

        // Create an array of promises to update participants in parallel
        const updatePromises = participants.map(async (participantObj) => {
            const participantCID = participantObj.connect_id;
            let path = bioKitMouthwash;
            switch(participantObj.visit) {
                case 'BL_1':
            ...
            const docId = snapshot.docs[0].id;
            return db.collection("participants").doc(docId).update({
                [`${collectionDetails}.${baseline}.${path}.${kitStatus}`]: addressPrinted
            });
        });
...
const assignKitToParticipant = async (data) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const kitData = {
            ...
            [kitStatus]: assigned,
            // Per documentation, collection round for replacement kits is still baseline
    ...
    try {
        ...
        await db.runTransaction(async transaction => {
            const { 
                ...
                kitStatus, shipped, shippedDateTime, preferredName, firstName
            } = fieldMapping;

            const kitAssemblyQuery = db.collection("kitAssembly").where(`${uniqueKitID}`, '==', shipmentData[uniqueKitID]);
            ...
            const kitData = {
                [kitStatus]: shipped,
                [shippedDateTime]: shipmentData[shippedDateTime]
            };


            // Needs multiple participant queries for all cases
            const participantQueryInitial = db.collection("participants")
                .where(`${collectionDetails}.${baseline}.${bioKitMouthwash}.${uniqueKitID}`, '==', shipmentData[uniqueKitID]);
            const participantQuery1 = db.collection("participants")
                .where(`${collectionDetails}.${baseline}.${bioKitMouthwashBL1}.${uniqueKitID}`, '==', shipmentData[uniqueKitID]);
            const participantQuery2 = db.collection("participants")
                .where(`${collectionDetails}.${baseline}.${bioKitMouthwashBL2}.${uniqueKitID}`, '==', shipmentData[uniqueKitID]);
            const [ptSnapshotInitial, ptSnapshot1, ptSnapshot2] = await Promise.all([
                transaction.get(participantQueryInitial), 
                transaction.get(participantQuery1),
                transaction.get(participantQuery2)]);
            const participantSnapshot = (ptSnapshot2.size && ptSnapshot2) ||
                                        (ptSnapshot1.size && ptSnapshot1) ||
                                        ptSnapshotInitial;
            ...
            if(baselineParticipantObject[bioKitMouthwashBL2]?.[uniqueKitID] === shipmentData[uniqueKitID]) {
                path = bioKitMouthwashBL2;
            } else if(baselineParticipantObject[bioKitMouthwashBL1]?.[uniqueKitID] === shipmentData[uniqueKitID]) {
                path = bioKitMouthwashBL1;
            }
            // const prevParticipantObject = participantDocData[collectionDetails][baseline][path];
            const uid = participantDocData['state']['uid'];
            const Connect_ID = participantDocData['Connect_ID'];
            const prefEmail = participantDocData[fieldMapping.prefEmail];
            const token = participantDocData['token'];
            const ptName = participantDocData[preferredName] || participantDocData[firstName]
            ...
            const updatedParticipantObject = {
                [`${collectionDetails}.${baseline}.${path}.${kitStatus}`]: shipped,
                [`${collectionDetails}.${baseline}.${path}.${shippedDateTime}`]: shipmentData[shippedDateTime]
            };
...
const storeKitReceipt = async (pkg) => {
    try {
        const {
            returnKitTrackingNum, kitStatus,
        ...
        await db.runTransaction(async (transaction) => {
            ...
            let ptSnapshotPromises = [
                transaction.get(db.collection("participants").where(`${collectionDetails}.${baseline}.${bioKitMouthwash}.${uniqueKitID}`, '==', kitDoc.data()[uniqueKitID])),
                transaction.get(db.collection("participants").where(`${collectionDetails}.${baseline}.${bioKitMouthwashBL1}.${uniqueKitID}`, '==', kitDoc.data()[uniqueKitID])),
                transaction.get(db.collection("participants").where(`${collectionDetails}.${baseline}.${bioKitMouthwashBL2}.${uniqueKitID}`, '==', kitDoc.data()[uniqueKitID]))
            ];
            const ptCandidates = await Promise.all(ptSnapshotPromises);
            const participantSnapshot = (ptCandidates[2]?.size && ptCandidates[2]) || 
                (ptCandidates[1].size && ptCandidates[1]) ||
                ptCandidates[0];
            const participantDoc = participantSnapshot.docs[0];
            ...
            const token = participantDocData['token'];
            const uid = participantDocData['state']['uid'];
            const site = participantDocData[healthCareProvider];
            const prefEmail = participantDocData[fieldMapping.prefEmail];
            const ptName = participantDocData[preferredName] || participantDocData[firstName]
            const surveyStatus = participantDocData[mouthwashSurveyCompletionStatus]
            const preferredLanguage = cidToLangMapper[participantDocData[fieldMapping.preferredLanguage]] || cidToLangMapper[fieldMapping.english];

            const path = (ptCandidates[2]?.size && bioKitMouthwashBL2) || 
                (ptCandidates[1].size && bioKitMouthwashBL1) ||
                bioKitMouthwash;
            // const prevParticipantObject = participantDocData[collectionDetails][baseline][path];
            const collectionId = pkg[collectionCupId]?.split(' ')[0];
            const objectId = pkg[collectionCupId]?.split(' ')[1];
            ...
            if(kitData[collectionCupId] !== pkg[collectionCupId]) {
            ...
            const biospecPkg = {
                [fieldMapping.tubesBagsCids.mouthwashTube1]: {
                    [tubeIsCollected]: yes,
                    [fieldMapping.objectId]: pkg[collectionCupId],
                    [receivedDateTime]: pkg[receivedDateTime]
                },
                [collectionAddtnlNotes]: pkg[collectionAddtnlNotes],
...
const validateKitReceiptCollectionDate = async (query) => {
    ...
    // If collected after it is received or if collected before it was shipped out to the participant
...
const pendingKitStatus = async () => {
    ...
    try { 
        const snapshot = await db.collection("kitAssembly")
                            ...
                            .orderBy(`${pendingDateTimeStamp}`, 'desc')
                            .select(
                                `${pendingDateTimeStamp}`,
                                `${returnKitTrackingNum}`,
                                `${supplyKitId}`,
...
const assignedKitStatusParticipants = async () => {
    try {
        ...
        console.error(error);
        throw new Error("Error in assignedKitStatusParticipants. ", error);
    }
...
/**
 * Applies a correction update to a participant document in Firestore.
 *
 * If the payload sets `state.148197146` to `'NULL'`, that field is deleted from the document. If the payload marks the participant as verified, missing survey statuses are initialized to 'not started'.
 *
 * @param {Object} correctionPayload - The correction payload. Must include a `token` field
 *   for participant lookup.
 * @returns {Promise<boolean>} `true` on success, `false` if no matching participant is found.
 */
const updateParticipantCorrection = async (correctionPayload) => {
    const snapshot = await db.collection('participants').where('token', '==', correctionPayload['token']).get();
    printDocsCount(snapshot, "updateParticipantCorrection");
    if (snapshot.empty) return false;

    const doc = snapshot.docs[0];
    const docData = doc.data();
    delete correctionPayload['token'];

    // When data correct tool (in SMDB) signals "not a duplicate", delete `state.148197146` field.
    if (correctionPayload['state.148197146'] === 'NULL') {
        correctionPayload['state.148197146'] = admin.firestore.FieldValue.delete();
    }

    if (Object.keys(correctionPayload).length > 0) {
        const dataToBeUpdated = checkSurveyStatusesWhenVerified(correctionPayload, docData);
        await doc.ref.update(dataToBeUpdated);
    }
    return true;
};
...
module.exports = {
    ...
    validateIDToken,
    verifyPin,
    linkParticipantWithFirebaseUID,
    linkInvitationRecordWithParticipantUID,
    participantExists,
    ...
    getPathologyBucketTierSuffix,
    isValidPathologyBucketName,
    getEhrDeliveries,
    updateEhrDeliveries,
    getMySamples,
    getAllMySamples,
