 ...
 * @param {object} firstSignInData - the request body with the participant's first sign-in timestamp. Optional: PIN number, and 'I do not have a PIN' flag.
 * @param {string} uid - the new participant's UID.
 */

const createParticipantRecord = async (res, firstSignInData, uid) => {
    const { createRecord, participantExists } = require('./firestore')
    ...
    try {
        ...
        // Validate items in body: firstSignInTimestanp, pinNumber, dontHavePinNumber
        const allowList = new Set([conceptIds.autogeneratedSignedInTime, conceptIds.pinNumber, conceptIds.iDoNotHaveAPIN].map(id => id.toString()));
        const invalidKeys = Object.keys(firstSignInData).filter(key => !allowList.has(key));
        
        if (invalidKeys.length > 0) {
            return res.status(400).json(getResponseJSON(`Invalid keys in createParticipantRecord: ${invalidKeys.join(', ')}`, 400));
        }

        const initialParticipantRecord = {
            state: {
                uid,
                ...defaultStateFlags
            },
            ['token']: uuid(),
            [conceptIds.participantMap.signedInFlag]: conceptIds.yes,
            [conceptIds.autogeneratedRecruitmentType]: conceptIds.passiveRecruit,
            [conceptIds.autogeneratedRecruitmentTypeTime]: new Date().toISOString(),
            ...defaultFlags,
            ...firstSignInData // (firstSignInTimestamp, entered pinNumber, iDoNotHaveAPIN)
        }

        const createRecordResponse = await createRecord(initialParticipantRecord);
        if (createRecordResponse instanceof Error) {
            console.error('Error in createParticipantRecord:', createRecordResponse);
            return res.status(500).json(getResponseJSON(createRecordResponse.message, 500));
        }
 ...
 * Other outcomes: duplicate account, invalid PIN (PIN doesn't match an existing record), or error.
 * @param {object} res - the response object.
 * @param {object} body - the request body with the participant-entered PIN number and first sign-in timestamp.
 * @param {string} uid - the participant's UID.
 * @returns {object} - the response object with the appropriate status code and message.
 */

const validatePin = async (res, body, uid) => {
    const { participantExists } = require('./firestore')
    const { verifyPin, linkInvitationRecordWithParticipantUID, updateResponse } = require('./firestore');
    ...
    try {
        ...
        const pin = body[conceptIds.pinNumber]?.trim();
        const firstSignInTimestamp = body[conceptIds.autogeneratedSignedInTime]?.trim();

        if (!pin || typeof pin !== 'string') {
            return res.status(400).json(getResponseJSON('Bad request: pin required and must be a string.', 400));
        }
...
const checkDerivedVariables = async (token, siteCode) => {
    ...
    if(Object.keys(updates).length > 0) {
        const { updateParticipantDataByDocId } = require('./firestore');
        await updateParticipantDataByDocId(doc, updates);
    }
}
...
const checkRefusalWithdrawals = (data) => {
    const anyRefusalWithdrawal = (
        ...
        data['685002411']['941963821'] === 353358909 ||
