...
/**
 * Update participant recruitment type based on incoming recruit-type state changes.
 *
 * @param {object} inputData - Incoming update payload.
 * @param {object} recordData - Existing participant record data.
 */
const updateRecruit = (inputData, recordData) => {
    const { updateRecruitType: updateField, autogeneratedRecruitmentType: recruitField, passiveToActiveUpdate, activeToPassiveUpdate, notActiveToPassiveUpdate, passiveRecruit, activeRecruit, notActiveRecruit } = fieldMapping;
    const updateValue = inputData[`state.${updateField}`];
    const recruitValue = recordData[recruitField];

    if (updateValue === passiveToActiveUpdate && recruitValue === passiveRecruit) inputData[recruitField] = activeRecruit;
    else if (updateValue === activeToPassiveUpdate && recruitValue === activeRecruit) inputData[recruitField] = passiveRecruit;
    else if (updateValue === notActiveToPassiveUpdate && recruitValue === notActiveRecruit) inputData[recruitField] = passiveRecruit;
};
...
const submitParticipantsData = async (req, res) => {
    ...
    for(let dataObj of dataArray){
        ...
        updateRecruit(dataObj, record.data);
        ...
        if(Object.keys(dataObj).length > 0) {
            ...
            await updateParticipantDataByDocId(docID, dataObj);
        }
...
const updateParticipantData = async (req, res, authObj) => {
    ...
    for(let dataObj of dataArray) {
        ...
        if (reinvitationCampaignType) {
            ...
            if (!docData['Connect_ID'] && docData[fieldMapping.autogeneratedRecruitmentType] === fieldMapping.activeRecruit && !docData[fieldMapping.reinvitationTimestamp] && Object.keys(flatUpdateObj).length === 1) {
                ...
                const participantStatusMessage = docData[fieldMapping.autogeneratedRecruitmentType] !== fieldMapping.activeRecruit ? ' Participant is not active.' : '';
        ...
        updateRecruit(flatUpdateObj, flatDocData);
        try {
            if (!dataObjError) {
                const promises = [];
                if (Object.keys(flatUpdateObj).length > 0) promises.push(updateParticipantDataByDocId(docID, flatUpdateObj));
